...
// ─── GET — fetch all agents under TSM with their monthly sales quotation targets ───────────────
export async function GET(req: Request) {
  try {
    ...
    // 2. Get all sales quotation rows for those agents for the given year
    ...
    // 3. Build a map: { [referenceid]: { [month]: { quote_target, quotation_amount_target } } }
    ...
    console.error("Error fetching TSM agent sales quotation targets:", err);
    return NextResponse.json(
...
// ─── PUT — upsert a single agent-month sales quotation target ─────────────────────────────────
export async function PUT(req: Request) {
  try {
    const { referenceid, month, year, quote_target, quotation_amount_target, tsm, manager } = await req.json();

    if (!referenceid || !month || !year || quote_target === undefined || quotation_amount_target === undefined) {
      return NextResponse.json(
    ...
    if (existing) {
      result = await supabase
        .from("sales_quotation")
        .update({
          quote_target: Number(quote_target),
          quotation_amount_target: Number(quotation_amount_target),
    ...
    console.error("Error upserting sales quotation target:", err);
    return NextResponse.json(
