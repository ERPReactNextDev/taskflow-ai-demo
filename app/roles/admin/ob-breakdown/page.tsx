...
function ObBreakdownContent() {
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    fetch(`/api/admin-agent-ob?year=${year}`)
  ...
  // ── Totals ──────────────────────────────────────────────────────────────────
  const getVal       = (ref: string, month: string) => obMap[ref]?.[month] ?? 0;
  ...
  return (
    <ProtectedPageWrapper>
      ...
      <SidebarInset className="overflow-hidden">
        ...
        <GlobalTopBar
          title="OB Calls Breakdown — System-wide"
          extra={
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                Outbound Touchbase call count per agent per month. Grouped by Manager → TSM.
              </p>
            </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    {MONTH_SHORT.map((m) => (
                      <th key={m} className="text-center px-2 py-3 font-bold uppercase tracking-wider text-gray-500 whitespace-nowrap min-w-[60px]">{m}</th>
                    ))}
                      ...
                      <tr key={`mgr-${mgr.managerId}`} className="bg-indigo-50 border-y border-indigo-200 cursor-pointer select-none" onClick={() => toggleManager(mgr.managerId)}>
                        ...
                        {MONTHS.map((month) => (
                          <td key={month} className="px-2 py-2.5 text-center font-bold text-indigo-600 text-[10px]">
                            {mgr.tsms.flatMap((t) => t.agents).reduce((s, a) => s + getVal(a.referenceid, month), 0) || "—"}
                          </td>
                        ))}
                        <td className="px-4 py-2.5 text-right font-black text-indigo-700">{managerTotal(mgr) || "—"}</td>
                      </tr>
                          ...
                          <tr key={`tsm-${tsm.tsmId}`} className="bg-slate-50 border-y border-slate-200 cursor-pointer select-none" onClick={() => toggleTsm(tsm.tsmId)}>
                            ...
                            {MONTHS.map((month) => (
                              <td key={month} className="px-2 py-2 text-center font-bold text-slate-500 text-[10px]">
                                {tsm.agents.reduce((s, a) => s + getVal(a.referenceid, month), 0) || "—"}
                              </td>
                            ))}
                            <td className="px-4 py-2 text-right font-bold text-slate-600 text-[10px]">{tsmTotal(tsm) || "—"}</td>
                          </tr>
                              ...
                              {MONTHS.map((month) => {
                                ...
                                return (
                                  <td key={month} className="px-2 py-2.5 text-center">
                                    <span className={val > 0 ? "font-semibold text-gray-700" : "text-gray-300"}>{val > 0 ? val : "—"}</span>
                                  </td>
                                );
                              })}
                ...
                {groups.length > 0 && (
                  <tfoot>
                    <tr className="border-t-2 border-gray-300 bg-gray-100">
                      ...
                      {MONTHS.map((month) => (
                        <td key={month} className="px-2 py-3 text-center font-bold text-gray-700 text-[10px]">{monthColTotal(month) || "—"}</td>
                      ))}
                      <td className="px-4 py-3 text-right font-black text-gray-900">{grandTotal || "—"}</td>
                    </tr>
                  </tfoot>
                )}
              </table>
