...
function QuotationTab({ manager, year }: { manager: string; year: string }) {
  ...
  const fetchTargets = useCallback(() => {
    ...
    fetch(`/api/manager-agent-sales-quotation?manager=${encodeURIComponent(manager)}&year=${year}`)
      ...
      .then((data) => {
        if (data?.success) { setAgents(data.agents ?? []); setTargets(data.targets ?? {}); }
      })
      .catch(() => {})
...
function AccountDevelopmentTab({ manager, year }: { manager: string; year: string }) {
  ...
  const fetchTargets = useCallback(() => {
    if (!manager) return;
    setLoading(true);
    // Reuse manager-agent-ob-target-sum pattern — but we need agents + targets from account_development
    // We use manager-fetch-tsms then tsm-agent-account-development per TSM
  ...
  const handleSave = async (referenceid: string, month: string, targetRaw: string, countRaw: string) => {
    ...
    try {
      const res = await fetch("/api/tsm-agent-account-development", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ referenceid, month, year, target, count, manager }),
      });
      if (!res.ok) throw new Error();
                    ...
                    <div className="relative flex items-center justify-center">
                      ...
                      <input type="text" defaultValue={vals.target > 0 ? String(vals.target) : ""} placeholder="—" className={inputCls}
                        onBlur={(e) => {
                          ...
                          if (newTarget !== (cur?.target ?? 0)) {
                            handleSave(agent.referenceid, month, e.target.value, String(cur?.count ?? 0));
                          }
                        }}
