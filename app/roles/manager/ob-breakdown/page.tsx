...
function TargetTab({
  agents, targets, year, manager, loading,
  onTargetUpdate,
}: {
  agents: Agent[];
  ...
  const handleSave = async (referenceid: string, month: string, rawValue: string) => {
    ...
    try {
      const res = await fetch("/api/tsm-agent-ob-target", {
        ...
        body: JSON.stringify({ referenceid, month, year, ob_target: parsed, manager }),
      });
...
function ObBreakdownContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/user?id=${encodeURIComponent(userId)}`)
      ...
      .then((data) => {
        if (data?.ReferenceID) setManager(data.ReferenceID);
      })
      .catch(() => {});
  }, [userId]);

  // Fetch OB Calls data (year view)
  const fetchObCalls = useCallback(() => {
    if (!manager) return;
    setLoadingOb(true);
    fetch(`/api/manager-agent-ob?manager=${encodeURIComponent(manager)}&year=${year}`)
      .then((r) => r.ok ? r.json() : null)
      .then((data) => {
        if (data?.success) {
  ...
  // Fetch Target data (year view) — reuses tsm-agent-ob-target via manager hierarchy
  const fetchTargets = useCallback(() => {
    if (!manager) return;
    setLoadingTarget(true);
    // Fetch all TSMs under the manager, then aggregate targets per agent
    fetch(`/api/manager-fetch-tsms?manager=${encodeURIComponent(manager)}`)
      ...
      .then(async (tsmData) => {
        ...
        if (tsms.length === 0) { setTargets({}); return; }
        ...
        // Merge agents + targets across all TSMs
        const mergedTargets: Record<string, Record<string, number>> = {};
        ...
        for (const d of results) {
          ...
          for (const [refId, monthMap] of Object.entries(d.targets ?? {})) {
            mergedTargets[refId] = { ...(mergedTargets[refId] ?? {}), ...(monthMap as Record<string, number>) };
          }
        }

        if (mergedAgents.length) setAgents(mergedAgents);
  ...
  // Fetch both on mount / year / manager change
  useEffect(() => {
    fetchObCalls();
    fetchTargets();
  }, [fetchObCalls, fetchTargets]);

  // Fetch outbound history for Touchbase / Outbound History tabs
