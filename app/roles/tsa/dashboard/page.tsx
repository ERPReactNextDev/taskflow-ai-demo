...
const BR_CARDS: (keyof CardVisibility)[] = [
  "brSummaryCards", "brSalesPipeline", "siteVisit",
  "databaseCoverage", "totalTimeSpent", "agentPerformanceDetailSingle",
];
...
function DashboardContent() {
  ...
  const fetchNewAccount = useCallback(async () => {
    ...
    try {
      const [countRes, targetRes] = await Promise.all([
        fetch(`/api/account-development-plan/count?referenceid=${encodeURIComponent(referenceid)}`),
      ...
      setNewAccountTarget(Number(targetData.target) || 2);
    } catch (err) {
  ...
  const fetchKpiMonthlyActuals = useCallback(async () => {
    ...
    try {
      // Reuse dashboard-tsa-performance — it already fetches everything with
      // correct month/YTD scoping, independent of the date-range picker.
      // We also pull ob/quote targets from sales_ob & sales_quotation in parallel.
      const now        = new Date();
      const year       = now.getFullYear().toString();
      const monthNames = ["January","February","March","April","May","June","July","August","September","October","November","December"];
      const month      = monthNames[now.getMonth()];

      const [perfRes, obTargetRes, quoteTargetRes] = await Promise.all([
        fetch(`/api/dashboard-tsa-performance?referenceid=${encodeURIComponent(referenceid)}`),
        fetch(`/api/sales-ob?referenceid=${encodeURIComponent(referenceid)}`),
        fetch(`/api/sales-quotation?referenceid=${encodeURIComponent(referenceid)}&year=${year}&month=${month}`),
      ]);

      const perf        = await perfRes.json();
      const obTarget    = await obTargetRes.json();
      const quoteTarget = await quoteTargetRes.json();

      if (perf.success) {
        // Build conversion counts from the TSA performance data.
        // dashboard-tsa-performance returns quoteToSO/soToSI counts directly.
        setKpiMonthlyActuals({
          totalActualSales:         Number(perf.runningSI)              || 0,
          obCallsCount:             Number(perf.obCalls)                || 0,
          obCallsTarget:            Number(obTarget.target)             || 0,
          quotesCount:              Number(perf.quotationsCount)        || 0,
          quotesTarget:             Number(quoteTarget.quoteTarget)     || 0,
          // callsToQuotes = same denominator as quoteToSOQuotationCount
          callsToQuotesCount:       Number(perf.quoteToSOQuotationCount)  || 0,
          quoteToSOQuotationCount:  Number(perf.quoteToSOQuotationCount)  || 0,
          quoteToSOSalesOrderCount: Number(perf.quoteToSOSalesOrderCount) || 0,
          soToSISalesOrderCount:    Number(perf.soToSISalesOrderCount)    || 0,
          soToSIDeliveredCount:     Number(perf.soToSIDeliveredCount)     || 0,
        });
      }
    } catch (err) {
  ...
  }, [fetchOutboundCalls, fetchOutboundCallsTarget, fetchApprovedQuotes, fetchCallsToQuotes, fetchQuoteToSO, fetchSoToSI, fetchClientVisits, fetchNewAccount, fetchSiSoAchievement, fetchSiteVisits, fetchSiteVisitTarget, fetchKpiMonthlyActuals]);
        ...
        <div className="flex flex-col gap-4 p-4">
          ...
          {visibility.kpiScores && (
            <KpiWeightedScores
              ...
              loading={loadingKpiMonthly || loadingSalesQuota}
              runningTarget={salesQuotaTotal}
              totalActualSales={kpiMonthlyActuals.totalActualSales}
              obCallsCount={kpiMonthlyActuals.obCallsCount}
              obCallsTarget={kpiMonthlyActuals.obCallsTarget || outboundCallsTarget}
              quotesCount={kpiMonthlyActuals.quotesCount}
              quotesTarget={kpiMonthlyActuals.quotesTarget || quoteTarget}
              callsToQuotesCount={kpiMonthlyActuals.callsToQuotesCount}
              obCallsForRatio={kpiMonthlyActuals.obCallsCount}
              quoteToSOSalesOrderCount={kpiMonthlyActuals.quoteToSOSalesOrderCount}
              quoteToSOQuotationCount={kpiMonthlyActuals.quoteToSOQuotationCount}
              soToSIDeliveredCount={kpiMonthlyActuals.soToSIDeliveredCount}
              soToSISalesOrderCount={kpiMonthlyActuals.soToSISalesOrderCount}
