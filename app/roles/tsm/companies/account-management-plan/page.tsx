...
import { AlertCircleIcon, Search, Eye } from "lucide-react";import { Spinner } from "@/components/ui/spinner";
...
function DashboardContent() {
      ...
      .then((data) => {
        if (data?.ReferenceID) {
          setUserDetails({
            ...
            tsm: data.ReferenceID, // TSM's own ReferenceID is the tsm column value
  ...
  // Fetch plans whenever tsm or searchTerm changes
  useEffect(() => {
    if (!userDetails.tsm) return;
    setLoadingPlans(true);
    const url = new URL("/api/tsm-account-development-plans-list", window.location.origin);
    url.searchParams.set("tsm", userDetails.tsm);
  ...
  }, [userDetails.tsm, searchTerm]);

  // Client-side filter on top of server results for instant feel
              ...
              <div className="flex items-center gap-2">
                <div className="relative flex-1 max-w-sm">
                  ...
                  <Input
                    ...
                    placeholder="Search company, manager, or agent..."
                          ...
                          <TableRow key={plan.id}>
                            ...
                            <TableCell className="text-right">
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => router.push(`/roles/tsm/companies/account-management-plan/${plan.id}?id=${encodeURIComponent(userId ?? "")}`)}
