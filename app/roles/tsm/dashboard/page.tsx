...
function DashboardContent() {
  ...
  const fetchPipeline = useCallback(async () => {
    ...
    try {
      ...
      const [quotesRes, callsToQuotesRes, quoteToSORes, soToSIRes, newAccCountRes, newAccTargetRes] = await Promise.all([
        ...
        // actual count from account_development_plans — YTD, no date range filter
        fetch(`/api/tsm-account-development-plans?tsm=${encodeURIComponent(referenceid)}`),
        // target from sales_account_development
        fetch(`/api/tsm-new-account-development?tsm=${encodeURIComponent(referenceid)}`),
      ]);

      const [quotesData, c2qData, q2soData, s2siData, newAccCountData, newAccTargetData] = await Promise.all([
        quotesRes.json(), callsToQuotesRes.json(), quoteToSORes.json(),
        soToSIRes.json(), newAccCountRes.json(), newAccTargetRes.json(),
      ]);
