...
export const ManagerSalesPipelineCard: React.FC<ManagerSalesPipelineCardProps> = ({
  ...
  const fetchTeamTargets = useCallback(async () => {
    ...
    try {
      ...
      if (dateRange?.from) dateParams.append("from", dateRange.from.toLocaleDateString("en-CA", { timeZone: "Asia/Manila" }));
      if (dateRange?.to)   dateParams.append("to",   dateRange.to.toLocaleDateString("en-CA",   { timeZone: "Asia/Manila" }));
