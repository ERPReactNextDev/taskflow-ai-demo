...
interface ManagerMonthlySiTrendCardProps {
  /** Manager ReferenceID — used as ?manager= in the API */
  manager: string;
}
...
export const ManagerMonthlySiTrendCard: React.FC<ManagerMonthlySiTrendCardProps> = ({ manager }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (view === "monthly") {
        ...
        const res  = await fetch(`/api/manager-monthly-si-trend?manager=${encodeURIComponent(manager)}&from=${from}&to=${to}`);
        if (!res.ok) throw new Error("Failed to fetch data");
        const data = await res.json();
        const full = MONTH_SHORT.map((label) => {
        ...
        const res     = await fetch(`/api/manager-monthly-si-trend?manager=${encodeURIComponent(manager)}&from=${from}&to=${to}&granularity=weekly`);
        if (!res.ok) throw new Error("Failed to fetch data");
        const data = await res.json();
        if (data.weeks) {
          ...
          for (const m of data.months ?? []) weekTotals[0] += m.total;
        ...
        const results = await Promise.all(
          years.map((y) =>
            fetch(`/api/manager-monthly-si-trend?manager=${encodeURIComponent(manager)}&from=${y}-01-01&to=${y}-12-31`)
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={points} margin={{ top: 12, right: 16, left: 8, bottom: 4 }}>
              ...
              <Bar dataKey="total" radius={[4, 4, 0, 0]}>
                {points.map((_, i) => (
                  <Cell key={i} fill={i === points.length - 1 ? "#1a5c3a" : "#a7c4b5"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={points} margin={{ top: 12, right: 16, left: 8, bottom: 4 }}>
              <defs>
                <linearGradient id="managerSiGradient" x1="0" y1="0" x2="0" y2="1">
              ...
              <Area
                ...
                stroke="#1a5c3a" strokeWidth={2.5}
                fill="url(#managerSiGradient)"
                dot={<CustomDot />}
