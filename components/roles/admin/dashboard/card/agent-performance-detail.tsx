...
interface AdminAgentPerformanceDetailProps { dateRange?: DateRange; }

interface AgentRow {
  referenceid: string; name: string;
  plan: number; siActual: number; soActual: number;
  obCalls: number; obCallsTarget: number;
  callsToQuote: number; quoteToSOQuotation: number; quoteToSOSalesOrder: number;
  soToSISalesOrder: number; soToSIDelivered: number;
  quotationAmountTarget: number; quotationAmount: number;
  siteVisits: number; siteVisitTarget: number;
  accountDevelopment: number; accountDevelopmentTarget: number;
  dbCoverageCovered: number; dbCoverageTotal: number;
  timeSpentMs: number; avgResponseTime: number;
  avgNonQuotationHT: number; avgQuotationHT: number; avgSpfHT: number;
}
...
function dbCoverageColor(score: number): string {
  if (score >= 90) return "#16a34a"; if (score >= 70) return "#10b981";
  if (score >= 50) return "#3b82f6"; if (score >= 30) return "#f59e0b";
  return "#ef4444";
}

// ── Component ─────────────────────────────────────────────────────────────────

export const AdminAgentPerformanceDetail: React.FC<AdminAgentPerformanceDetailProps> = ({ dateRange }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res  = await fetch(`/api/admin-agent-performance?${params.toString()}`);
  ...
  const totals = agents.reduce(
    (acc, a) => ({
      plan: acc.plan + a.plan, siActual: acc.siActual + a.siActual, soActual: acc.soActual + a.soActual,
      obCalls: acc.obCalls + a.obCalls, obCallsTarget: acc.obCallsTarget + a.obCallsTarget,
      ...
      siteVisits: acc.siteVisits + a.siteVisits, siteVisitTarget: acc.siteVisitTarget + a.siteVisitTarget,
    ...
    { plan:0,siActual:0,soActual:0,obCalls:0,obCallsTarget:0,callsToQuote:0,quoteToSOQuotation:0,
      quoteToSOSalesOrder:0,soToSISalesOrder:0,soToSIDelivered:0,quotationAmountTarget:0,
      quotationAmount:0,siteVisits:0,siteVisitTarget:0,accountDevelopment:0,
      accountDevelopmentTarget:0,timeSpentMs:0 }
  );
      ...
      <CardContent className="p-4">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-500">
            Agent Performance Detail — System View
          </p>
        ...
        {hasFetched && (
          <div className={`overflow-x-auto transition-all duration-300 relative ${loading ? "blur-sm opacity-50 pointer-events-none" : ""}`}>
            {!loading && !error && agents.length === 0 && (
              <p className="text-xs text-gray-400 text-center py-8">No active agents found under this manager.</p>
            )}
            {agents.length > 0 && (
              <table className="min-w-full text-xs relative">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-2 px-1 font-bold text-gray-500 whitespace-nowrap sticky left-0 bg-white z-10 min-w-[140px]">Agent</th>
                ...
                <tbody>
                  {agents.map((agent) => (
                    <tr key={agent.referenceid} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors whitespace-nowrap">
                      <td className="py-2.5 px-1 sticky left-0 bg-white hover:bg-gray-50/50 z-10">
                        <div className="flex items-center gap-1">
                          <User className="w-3 h-3 text-gray-400 shrink-0" />
                          <span className="font-medium text-gray-800 uppercase">{agent.name}</span>
                        </div>
                      </td>
                      ...
                      <td className={tdCls}>{agent.callsToQuote>0||agent.obCalls>0?`${agent.callsToQuote} (${agent.obCalls>0?Math.round((agent.callsToQuote/agent.obCalls)*100):0}%)`:"—"}</td>
                      <td className={tdCls}>{agent.quoteToSOQuotation>0?`${agent.quoteToSOSalesOrder}/${agent.quoteToSOQuotation} (${Math.round((agent.quoteToSOSalesOrder/agent.quoteToSOQuotation)*100)}%)`:"—"}</td>
                      <td className={tdCls}>{agent.soToSISalesOrder>0?`${agent.soToSIDelivered}/${agent.soToSISalesOrder} (${Math.round((agent.soToSIDelivered/agent.soToSISalesOrder)*100)}%)`:"—"}</td>
                      ...
                      <td className={tdCls}>
                        <span style={{ color:dbCoverageColor(agent.dbCoverageTotal>0?Math.round((agent.dbCoverageCovered/agent.dbCoverageTotal)*100):0) }} className="font-medium">
                ...
                <tfoot>
                  <tr className="border-t-2 border-gray-200 bg-gray-50 font-bold whitespace-nowrap">
                    <td className="py-2.5 px-1 text-xs font-black uppercase tracking-widest text-gray-600 sticky left-0 bg-gray-50 z-10">Team Total</td>
                    ...
                    <td className={tdCls}>{totals.callsToQuote>0||totals.obCalls>0?`${totals.callsToQuote} (${totals.obCalls>0?Math.round((totals.callsToQuote/totals.obCalls)*100):0}%)`:"—"}</td>
                    <td className={tdCls}>{totals.quoteToSOQuotation>0?`${totals.quoteToSOSalesOrder}/${totals.quoteToSOQuotation} (${Math.round((totals.quoteToSOSalesOrder/totals.quoteToSOQuotation)*100)}%)`:"—"}</td>
                    <td className={tdCls}>{totals.soToSISalesOrder>0?`${totals.soToSIDelivered}/${totals.soToSISalesOrder} (${Math.round((totals.soToSIDelivered/totals.soToSISalesOrder)*100)}%)`:"—"}</td>
